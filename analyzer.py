from model_manager import (
    ...
    run_risk_agent,
    run_summary_agent,
    extract_final_verdict  # ✅ make sure this is included
)

def analyze_rfp(rfp_text: str, company_profile: str) -> dict:
    results = {}

    # Step 1: Run Verdict Agent
    eligibility_response = run_verdict_agent(rfp_text, company_profile)
    final_verdict = extract_final_verdict(eligibility_response)

    results['Eligibility Verdict'] = final_verdict
    results['Verdict Full Response'] = eligibility_response.strip()
